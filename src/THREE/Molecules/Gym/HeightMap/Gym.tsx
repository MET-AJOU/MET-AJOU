...
const HeightMap = ({ elementSize, position, rotation }: HeightmapProps) => {
  const heightmap = useTexture("/models/AjouMap/HeightMap/gym_heightmap_1024.png");
  const heights = useAsset<number[][], Texture[]>(async () => createHeightfieldMatrix(heightmap.image), heightmap).map((items) =>
    items.map((item) => {
      if (item > 3) return item * 0.04;
      if (item > 2) return item * 0.04;
      if (item > 1.5) return item * 0.04;
      if (item > 1) return item * 0.04;
      if (item > 0.5) return item * 0.04;
      return item * 0.04;
    })
  );
