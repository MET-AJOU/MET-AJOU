...
const HeightMap = ({ elementSize, position, rotation }: HeightmapProps) => {
  const heightmap = useTexture("/models/AjouMap/HeightMap/classroom_heightmap_1024.png");
  const heights = useAsset<number[][], Texture[]>(async () => createHeightfieldMatrix(heightmap.image), heightmap).map((items) =>
    items.map((item) => {
      if (item > 3) return item * 0.18;
      if (item > 2) return item * 0.32;
      if (item > 1.5) return item * 0.2;
      if (item > 1) return item * 0.22;
      return item * 0.22;
    })
  );
