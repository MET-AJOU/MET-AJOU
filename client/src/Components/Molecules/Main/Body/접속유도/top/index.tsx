...
const HomeInfo = () => {
  ...
  useEffect(() => {
    if (!isVisible) return;
    const childrens = document.querySelector(".cont")?.querySelectorAll(".down");
    if (!childrens) return;

    setTimeout(() => {
      childrens[0].classList.remove("down");
      childrens[0].classList.add("up_first");
    }, 1000);

    setTimeout(() => {
      childrens[1].classList.add("up_second");
      childrens[1].classList.remove("down");
    }, 2000);
    setTimeout(() => {
      childrens[2].classList.add("up_third");
      childrens[2].classList.remove("down");
    }, 3000);
  }, [isVisible]);
  ...
  return (
    <HomeInfoContainer className="cont" ref={ref}>
      ...
      <InfoContainer>
        <BlueBox />
        <InfoTitle className="down">
          <BlueText>아주대학교</BlueText>
          <span>의 가상세계로</span>
          <p>여정을 떠날 준비가 되었나요?</p>
        </InfoTitle>
        <TextContainer className="down">
        ...
        <StartButton color="blue" className="down" />
      </InfoContainer>
    </HomeInfoContainer>
  );
};
